...
public class RewardCalculationImpl implements IRewardCalculationService {
    ...
    public List<DataSetDto> getLast3MonthsRewardList(String customerId) {
        ...
        return this.dataSetDtoList.stream().filter(transaction -> transaction.getPurChaseDate().isBefore(dateThrreMonthBack)
                && transaction.getCustomerId().equalsIgnoreCase(customerId)).collect(Collectors.toList());
